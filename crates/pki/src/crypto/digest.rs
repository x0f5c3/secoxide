...
use digest::{FixedOutputReset, Output, Reset};
use digest::typenum::U32;


pub enum DigestAlgos {
    SHA256,
    BLAKE3,
}
...
pub struct DigestAlgorithm<T: Digest> {
    algo: DigestAlgos,
    ctx: T,
}

impl<T: Digest> DigestAlgorithm<T> {
    /// Creates a value representing the SHA-256 algorithm.
    pub fn sha256() -> Self {
        DigestAlgorithm {
            algo: DigestAlgos::SHA256,
            ctx: sha2::Sha256::new(),
        }
    }
    ...
    pub fn is_sha256(&self) -> bool {
        matches!(self.algo, DigestAlgos::SHA256)
    }
    pub fn blake3() -> Self {
        DigestAlgorithm {
            algo: DigestAlgos::BLAKE3,
            ctx: blake3::Hasher::new(),
        }
    }
    pub fn is_blake3(&self) -> bool {
        matches!(self.algo, DigestAlgos::BLAKE3)
    }
...
impl<T: Digest> digest::OutputSizeUser for DigestAlgorithm<T> {
    type OutputSize = U32;
}

impl<T: Digest + digest::Update> digest::Update for DigestAlgorithm<T> {
    ...
    fn chain(self, data: impl AsRef<[u8]>) -> Self where Self: Sized {
        Self {
            algo: self.algo,
            ctx: self.ctx.chain(data)
        }
    }
}


impl<T: Digest + digest::FixedOutput + digest::Update> digest::FixedOutput for DigestAlgorithm<T> {
    fn finalize_into(self, out: &mut Output<Self>) {
        self.ctx.finalize_into(out)
    }

    fn finalize_fixed(self) -> Output<Self> {
        self.ctx.finalize_fixed()
    }
}

impl<T: Digest + digest::HashMarker> digest::HashMarker for DigestAlgorithm<T> {}
...
impl<T: Digest> DigestAlgorithm<T> {
    ...
    pub fn digest(self, data: &[u8]) -> [u8;32]  {
        sha2::Sha256::digest(data).0.into()
    }

    /// Calculates the digest for the content of a file.
    pub fn digest_file(
        self, path: impl AsRef<Path>
    ) -> Result<Output<Self>, io::Error> {
